...
def find_dockpack_bin() -> str:
    """Return the dockpack binary path."""

    dockpack_exe = "dockpack" + sysconfig.get_config_var("EXE")

    scripts_path = os.path.join(sysconfig.get_path("scripts"), dockpack_exe)
    ...
    user_path = os.path.join(
        sysconfig.get_path("scripts", scheme=user_scheme), dockpack_exe
    )
    ...
    target_path = os.path.join(pkg_root, "bin", dockpack_exe)
    if os.path.isfile(target_path):
        return target_path

    # Search for pip-specific build environments.
    #
    # Expect to find dockpack in <prefix>/pip-build-env-<rand>/overlay/bin/dockpack
    ...
    if len(paths) >= 2:
        ...
        if (
            ...
            # The overlay must contain the dockpack binary.
            candidate = os.path.join(paths[0], dockpack_exe)
...
if __name__ == "__main__":
    dockpack = os.fsdecode(find_dockpack_bin())
    if sys.platform == "win32":
        import subprocess

        completed_process = subprocess.run([dockpack, *sys.argv[1:]])
        sys.exit(completed_process.returncode)
    else:
        os.execvp(dockpack, [dockpack, *sys.argv[1:]])